...
class AttentionPool(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        if x.shape[1] != self.variables["params"]["positional_embedding"].shape[0]:
            ...
            positional_embedding = self.param(
                ...
                jax.nn.initializers.normal(1.0 / x.shape[-1] ** 0.5),
                (x.shape[1], x.shape[2]),
            )
...
class ResidualAttentionBlock(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, attn_mask=None) -> jnp.ndarray:
        ...
        x = x + nn.SelfAttention(self.num_heads, name="attn", deterministic=True)(
            xn, attn_mask
        )
...
class Transformer(nn.Module):
    ...
    def __call__(
        ...
        for i in range(self.num_layers):
            x = ResidualAttentionBlock(num_heads=self.num_heads, name=f"resblocks.{i}")(
                x, attn_mask
            )
...
class VisionTransformer(nn.Module):
    ...
    def __call__(
        ...
        x = jnp.concatenate(
            (jnp.tile(class_embedding[None, None, :], (x.shape[0], 1, 1)), x), axis=1
        )
        if x.shape[1] != self.variables["params"]["positional_embedding"].shape[0]:
            positional_embedding = self.variables["params"]["positional_embedding"]
            positional_embedding = positional_embedding[: x.shape[1]]
